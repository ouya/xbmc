...
const uint32_t avpriv_dca_channel_count[16] =
{
    1, // Mono
    2, // dual mono
    2, // stereo
    2, // sum and diff
    2, // left and right total
    3, // C + L + R
    3, // L + R + S
    4, // C + L + R + S
    4, // L + R + SL + SR
                            ...
                            int *sample_rate, int* channels)
{
    ...
    int ret, sample_blocks, sr_code, frame_size, amode;
    ...
    *duration = 256 * (sample_blocks / 8);
    ...
    frame_size = get_bits(&gb, 14) + 1;
    amode = get_bits(&gb, 6);

    *channels = avpriv_dca_channel_count[amode];
                     ...
                     const uint8_t * buf, int buf_size)
{
    DCAParseContext *pc1 = s->priv_data;
    ParseContext *pc = &pc1->pc;
    int next, duration, sample_rate, channels;
    ...
    if (!dca_parse_params(buf, buf_size, &duration, &sample_rate, &channels)) {
//        s->duration = duration;
        avctx->sample_rate = sample_rate;
        avctx->channels = channels;
        av_log(NULL, AV_LOG_INFO, "DCA_PARSER: Read duration %d", duration);
    } else {
//        s->duration = 0;
    }
